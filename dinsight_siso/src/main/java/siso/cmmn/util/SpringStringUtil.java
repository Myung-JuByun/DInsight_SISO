...
/**
	...
	/**
	 * ajax 전송시 formData로 넘어오는 경우 한글이 깨지는 경우가 있는데 깨진 한글을 원래대로 변환하는 함수 
	 */
	public static String convertFormData(String srcString) {
		String returnStr = "";
		
		try {
			if(StringUtils.isNotEmpty(srcString))
				returnStr = new String(srcString.getBytes("8859_1"), "UTF-8");
		} catch (Exception e) {
