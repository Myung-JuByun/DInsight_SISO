		...
		try {
			...
			while(file_iter.hasNext()){
				...
				if(mFile.getSize()>0){
					...
					String newName = "";
					String stordFilePath = SpringProperties.getProperty("Globals.fileStorePath");								        

					//newName 은 Naming Convention에 의해서 생성
					newName = SpringStringUtil.getTimeStamp();	// 2012.11 KISA 보안조치
					writeFile(mFile, newName, stordFilePath);

					String pdfFileSrcValue = stordFilePath.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR);
					File inputFile = new File(SpringWebUtil.filePathBlackList(pdfFileSrcValue + newName));
