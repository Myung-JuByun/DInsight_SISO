...
public class SpringXMLDoc {
    ...
    // Log
    protected static final Log log = LogFactory.getLog(SpringXMLDoc.class);
    /**
	...
	*/
	public static boolean creatSchemaToClass(String xml, String ja) throws Exception {
		...
		Process p = null;
		String cmdStr = SpringProperties.getPathProperty(Globals.SHELL_FILE_PATH, "SHELL."+Globals.OS_TYPE+".compileSchema");
	...
	*/
	public static SndngMailDocument getXMLToClass(String file) throws Exception {
        ...
        try {
        	...
        	xmlFile = new File(SpringWebUtil.filePathBlackList(file1));
	        if (xmlFile.exists() && xmlFile.isFile()) {
	...
	*/
	public static boolean getClassToXML(SndngMailDocument mailDoc, String file) throws Exception {
		...
		try {
			...
			String file1 = file.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR);
			file1 = SpringFileTool.createNewFile(file1);
			File xmlFile = new File(SpringWebUtil.filePathBlackList(file1));
