/**
 *  Class Name : SpringNetworkState.java
...
public class SpringNetworkState
{
			...
			if ("WINDOWS".equals(Globals.OS_TYPE)) {
				...
				while (true) {
	        	    ...
	        	    if (str == null) break;
	 					...
	 					if (str.length() >= MAX_STR_LEN)	throw new java.lang.Exception ("input too long");	 					
						if (!str.trim().equals(""))	processes.add(str);						
					}
				}
				
				input.close();
			} else if ("UNIX".equals(Globals.OS_TYPE)){
				String cmdStr = SpringProperties.getPathProperty(Globals.SERVER_CONF_PATH, "SHELL."+Globals.OS_TYPE+".getNetWorkInfo");
	...
	public static String getMyIPaddress()
	{
		try {
			...
			if (!SpringWebUtil.isIPAddress(InetAddress.getLocalHost ().getHostAddress()))
	...
	public static boolean getPingTest(String requestIP) throws Exception {
		...
		if (!SpringWebUtil.isIPAddress(requestIP))
