...
import siso.cmmn.util.SpringWebUtil;
import siso.cmmn.util.Globals;

public class SpringFileTool {
    ...
    // Log
    protected static final Log log = LogFactory.getLog(SpringFileTool.class);
    ...
    public static boolean getExistDirectory(String targetDirPath) throws Exception {
		...
		File f = null;
		try {
		    f = new File(SpringWebUtil.filePathBlackList(targetDirPath));
    ...
    public static ArrayList getExistDirectory(String baseDirPath, String targetDirPath, int cnt) throws Exception {
		...
		try {
		    f = new File(SpringWebUtil.filePathBlackList(baseDirPath));
		    isExist = f.exists();
			...
			f = new File(SpringWebUtil.filePathBlackList(subDirPath));
			...
			if (targetDirPath.equals(dirList[i])) {
			    ...
			    if (new File(SpringWebUtil.filePathBlackList(baseDirPath) + "/" + dirList[i]).isDirectory()) {
				dirFindCnt++;
	...
	String result = "";
	try {
	    f = new File(SpringWebUtil.filePathBlackList(filePath));
    ...
    public static ArrayList getLastDirectoryForModifiedDate(String baseDirPath, String fromDate, String toDate) {
		...
		try {
		    ...
		    for (int i = 0; i < subDirList.length; i++) {
				...
				childFile = new File(SpringWebUtil.filePathBlackList(subDirPath));
    ...
    public static String deletePath(String filePath) {
		File file = new File(SpringWebUtil.filePathBlackList(filePath));
		String result = "";
		try {
    ...
    public static String createDirectory(String dirPath) {
		File file = new File(SpringWebUtil.filePathBlackList(dirPath));
		String result = "";
		try {	    
		    if (!file.exists()) {
				file.createNewFile();
    ...
    public static boolean checkFileExstByName(String dir, String file) throws Exception {
		...
		File srcDrctry = new File(SpringWebUtil.filePathBlackList(drctry));
    ...
    public static ArrayList getSubFilesByOwner(File[] fileArray, String owner) throws Exception {
		...
		for (int i = 0; i < fileArray.length; i++) {
			...
			// 파일 생성자 조회
				String fullpath = SpringWebUtil.filePathBlackList(fileArray[i].getAbsolutePath());
				Process p = null;
				if (Globals.OS_TYPE.equals("UNIX")) {
				    String[] command = { SpringProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getDrctryByOwner"),
		...
		try {
		    ...
		    if (file.exists()) {
				...
				if (file.isFile()) {
				    ...
				    if (file.mkdirs())	result = file.getAbsolutePath();				    
				} else {
				    result = file.getAbsolutePath();
				}
		    } else {
    ...
    public static String deleteDirectory(String dirDeletePath) {
		...
		String result = "";
		File file = new File(SpringWebUtil.filePathBlackList(dirDeletePath));
		if (file.isDirectory()) {
		    String[] fileList = file.list();
		    //소속된 파일을 모두 삭제
		    for (int i = 0; i < fileList.length; i++) {
				log.debug("fileList["+i+"] : "+ dirDeletePath +"/"+fileList[i]);
				File f = new File(SpringWebUtil.filePathBlackList(dirDeletePath) + "/" + fileList[i]);
    ...
    public static String deleteFile(String fileDeletePath) {
		...
		String result = "";
		File file = new File(SpringWebUtil.filePathBlackList(fileDeletePath));		
		if (file.isFile())	result = deletePath(fileDeletePath);		
    ...
    public static boolean checkReadAuth(String file) throws Exception {
		...
		try {
		    ...
		    File srcFile = new File(SpringWebUtil.filePathBlackList(file1));
    ...
    public static ArrayList getFileListByDate(String drctry, String updtDate) throws Exception {
		...
		File file = new File(SpringWebUtil.filePathBlackList(drctry1));
    ...
    public static Vector parsFileByChar(String parFile, String parChar, int parField) throws Exception {
		...
		File file = new File(SpringWebUtil.filePathBlackList(parFile1));
		...
		try {
		    ...
		    if (file.exists() && file.isFile()) {
				...
				String[] strArr = SpringStringUtil.split(strBuff.toString(), parChar);
				...
				for (int i = 0; i < strArr.length; i++) {	
				    if (parField != 1) {
						if ((filedCnt % parField) == 1) {
						    ...
						    if (strArr[i] != null) {	// 2011.10.21 보안점검 후속조치
								arr.add(strArr[i]);
							    if (i == (strArr.length - 1))	parResult.add(arr);						    
						    }
						}
				    } else {
						arr = new ArrayList();
    ...
    public static boolean cmprFilesBySize(String cmprFile1, String cmprFile2) throws Exception {
		...
		File file1 = new File(SpringWebUtil.filePathBlackList(cmprFile11));
		File file2 = new File(SpringWebUtil.filePathBlackList(cmprFile22));
    ...
    public static boolean cmprFilesByUpdtPd(String cmprFile1, String cmprFile2) throws Exception {	
		...
		if (file1.exists() && file2.exists() && file1.isFile() && file2.isFile()) {
		    ...
		    // 수정일자 비교
		    if (lastUpdtDate1.equals(lastUpdtDate2))	result = true;		    
		}
		    ...
		    if (file1.exists() && file2.exists() && file1.isFile() && file2.isFile()) {
				...
				for (int i = 0; i < cmprText1.size(); i++) {
				    if (!isWrong) { //   if(isWrong != true){
						...
						if (!text1.equals(text2))	isWrong = true;						
				    }
				}
    ...
    public static boolean cmprFilesByOwner(String cmprFile1, String cmprFile2) throws Exception {
		boolean result = false;			// 파일 동일 여부
		String owner1 = getOwner(cmprFile1);			// 파일1 생성자
		String owner2 = getOwner(cmprFile2);			// 파일2 생성자
    ...
    public static boolean copyFile(String source, String target) throws Exception {
		...
		File srcFile = new File(SpringWebUtil.filePathBlackList(src));
		...
		try {
			...
			//log.debug("tar:"+tar);
			File tarFile = new File(SpringWebUtil.filePathBlackList(tar));
    ...
    public static boolean copyFiles(String[] source, String target) throws Exception {
		...
		for (int i = 0; i < source.length; i++) {
		    ...
		    File chkFile = new File(SpringWebUtil.filePathBlackList(src));
		...
		for (int j = 0; j < source.length; j++) {
		    if (result) { //result != false
				// 타겟파일이름 명명
				File chkFile = new File(SpringWebUtil.filePathBlackList(source[j]));
				String tarTemp = tar + FILE_SEPARATOR + chkFile.getName();
	
				try {
				    // 복사될 target 파일 생성
				    tarTemp = createNewFile(tarTemp);
				    File tarFile = new File(SpringWebUtil.filePathBlackList(tarTemp));
		...
		if (srcFile.exists() && srcFile.isDirectory()) {	
		    ...
		    for (int i = 0; i < list.size(); i++) {
				if (result) { //f(result != false){
				    ...
				    // 타겟파일이름 명명
				    File chkFile = new File(SpringWebUtil.filePathBlackList(abspath));
				    String tarTemp = tar + FILE_SEPARATOR + chkFile.getName();
    ...
    public static boolean execCopyFile(File srcFile, File tarFile) throws Exception {
		...
		try {
		    ...
		    if (tarFile1.isDirectory())
		    	tarFile1 = new File(SpringWebUtil.filePathBlackList(tarFile1.getAbsolutePath()) + "/" + srcFile.getName());
    ...
    public static String getMountLc(String file) throws Exception {
		...
		try {
		    ...
		    if (srcFile.exists()) {
				...
				if (Globals.OS_TYPE.equals("UNIX")) {
				    Process p = null;
				    String[] command = { SpringProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getMountLc"), src, "/" };
				    p = Runtime.getRuntime().exec(command);
				    //p.waitFor();
    ...
    public static String getOwner(String file) throws Exception {
		...
		try {
		    ...
		    if (srcFile.exists()) {
				...
				Process p = null;
				String cmdStr = SpringProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getDrctryOwner");
    ...
    public static String getAccess(String file) throws Exception {
		...
		try {
		    ...
		    if (srcFile.exists()) {	
				// 접근권한 조회
				String parentPath = SpringWebUtil.filePathBlackList(srcFile.getParent());
				String fname = SpringWebUtil.filePathBlackList(srcFile.getName());		
				Process p = null;
				
				if (Globals.OS_TYPE.equals("UNIX")) {
				    String[] command = { SpringProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".getDrctryAccess"),
					    parentPath, fname };
				    p = Runtime.getRuntime().exec(command);
				    p.waitFor();
				    //프로세스 에러시 종료
		...
		try {
		    ...
		    if (srcFile.exists()) {	
				...
				if (strArr.length >= 2) {
				    ...
				    type = SpringProperties.getProperty(Globals.FILE_FORMAT_PATH, format);
				}
		    }
		} catch (Exception ex) {
		    //ex.printStackTrace();
    ...
    public static boolean copyDirectory(String originalDirPath, String targetDirPath) throws Exception {
		...
		try {
		    f = new File(SpringWebUtil.filePathBlackList(originalDirPath));
		    // 원본이 유효해야 진행한다.
		    if (f.exists() && f.isDirectory()) {	
				...
				if (targetDirPath1.equals("")) {
				    ...
				    File targetDir = new File(SpringWebUtil.filePathBlackList(targetDirPath1));
				    targetDir.mkdirs();
				    // 디렉토리에 속한 파일들을 복사한다.
				    String[] originalFileList = f.list();
				    if (originalFileList.length > 0) {
						for (int i = 0; i < originalFileList.length; i++) {
						    File subF = new File(SpringWebUtil.filePathBlackList(originalDirPath) + FILE_SEPARATOR + originalFileList[i]);
    ...
    public static boolean copyDirectory(String originalDirPath, String targetDirPath, String owner) throws Exception {
		...
		try {
		    ...
		    if (realOwner.equals(owner))	isInCondition = true;
    ...
    public static long getDirectorySize(String targetDirPath) throws Exception {
		File f = new File(SpringWebUtil.filePathBlackList(targetDirPath));
		...
		for (int i = 0; i < list.length; i++) {	
		    if (list[i].isDirectory())	// 디렉토리 안에 디렉토리면 그 안의 파일목록에서 찾도록 재귀호출한다.				
    ...
    public static boolean moveFile(String originalDirPath, String targetDirPath) throws Exception {
		...
		try {
		    ...
		    if (f.exists()) {
				// 타겟으로 설정한 경로가 유효한지 확인(중간경로에 파일명 이 포함되어있으면 유효하지 못하므로 진행안함.
				File targetDir = new File(SpringWebUtil.filePathBlackList(targetDirPath));
				if (targetDir.exists()) {
				    // 타겟경로가 이미 있는 경우는 종료
				    result = false;
				} else {
				    // 새로 생성되는 경우만 진행한다. (이동쉘을 실행시킨다.)
				    String cmdStr = SpringProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL." + Globals.OS_TYPE + ".moveDrctry");
				    String[] command = { cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
					    SpringWebUtil.filePathBlackList(originalDirPath.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR)),
					    SpringWebUtil.filePathBlackList(targetDirPath.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR)) };
    ...
    public static boolean startDirectoryMonitering(String targetDirPath) throws Exception {
		...
		try {
		    ...
		    File targetF = new File(SpringWebUtil.filePathBlackList(targetDirPath));
		    File logF = new File(SpringWebUtil.filePathBlackList(Globals.CONF_PATH) + "/" + targetF.getName() + ".log");
		    ...
		    if (targetF.exists() && targetF.isDirectory()) {
				if (logF.exists()) {
					...
					if (lastStr.equals("END")) {
						SpringFileMntrg t = new SpringFileMntrg(SpringWebUtil.filePathBlackList(targetDirPath), logF);
						t.start();
				    }
				} else {
				    result = logF.createNewFile();
				    SpringFileMntrg t = new SpringFileMntrg(targetDirPath, logF);
				    t.start();
				}
		    }
		} catch (Exception e) {
