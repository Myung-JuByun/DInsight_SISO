...
import siso.cmmn.mail.service.AtchmnFileVO;
...
import siso.cmmn.util.sim.SpringXMLDoc;
 ...
 */
@Service
public class SpringSndngMailRegistServiceImpl implements SpringSndngMailRegistService {

	@Autowired
	public SqlSession sqlSession;	
	public SndngMailRegistMapper sndngMailRegistMapper;
    
    /** Message ID Generation */       
	public EgovIdGnrService egovMailMsgIdGnrService;
    
    @Autowired
    public SpringSndngMailService springSndngMailService;
    
    public SpringSndngMailRegistServiceImpl(SqlSession sqlSession, SpringSndngMailService springSndngMailService){
    	this.sqlSession = sqlSession;
    	this.sndngMailRegistMapper = sqlSession.getMapper(SndngMailRegistMapper.class);
    	this.springSndngMailService = springSndngMailService;
    }
    ...
    public boolean insertSndngMail(SndngMailVO vo) throws Exception {
    	...
    	for (int j = 0; j < recptnPersonList.length; j++) {
        	...
        	// 1-3.발송메일을 등록한다.
        	sndngMailRegistMapper.insertSndngMail(mailVO);
        	
        	// 1-4.메일을 발송한다.
        	boolean sendingMailResult = springSndngMailService.sndngMail(mailVO);
        	
        	if(!sendingMailResult) {
        		mailVO.setSndngResultCode("F");	// 발송결과 실패
            	sndngMailRegistMapper.updateSndngMail(mailVO);	// 발송상태를 DB에 업데이트 한다.
        		return false;
        	}
	...
	public boolean trnsmitXmlData(SndngMailVO vo) throws Exception {
    	...
    	String streFileList = "";
    	List atchmnFileList = sndngMailRegistMapper.selectAtchmnFileList(vo);
        ...
        boolean result = SpringXMLDoc.getClassToXML(mailDoc, xmlFile);
        if (result == true) {
    ...
    public boolean recptnXmlData(String xmlFile) throws Exception {
    	...
    	SndngMailDocument mailDoc = SpringXMLDoc.getXMLToClass(xmlFile);
    	...
    	// 2. DB에 업데이트 한다.
    	sndngMailRegistMapper.updateSndngMail(sndngMailVO);
