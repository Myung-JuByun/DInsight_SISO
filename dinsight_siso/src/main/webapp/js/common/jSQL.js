function jSQL(){
	...
	this.parse=function(str){
		...
		var proc=str;
		var opt=cf.getHead(proc,6);
		var strBody=cf.cutHead(proc,6);
	...
	function procSelect(str){
		...
		//step5. verify cols' validation and change it to index;
		var idc=[];
		...
		//step6. extract cols from result;
	...
	function procJoin(tblname){
		...
		if(numJoin>1){
			...
			var aft=cmds.getClip(idxCmd,cmds.length-1);
			l.reg(
				"JOIN", 
				procJoin(bfr.join(" ")	)
			);
		...
		//join command filtering process!!///////////////////////////////////////////////
		if(numJoin=="0"){
			optr="cross join";
		...
		if(optr=="left outer join"){
			...
			var hdr=hdrConcat(ftbl,stbl), meta=arrConcat(getTable(ftbl).meta, getTable(stbl).meta);
			var obj={},error=false;
			agr.split("=").trav(function(d,i){
				...
				var tmp=d.split(".");
				var nt=tmp[0].trimFB(), nc=tmp[1].trimFB();
				if(!getTable(nt)){
					errorMessage(0,nt);
					error=true;
					return true;
		...
		if(optr=="right outer join"){
			...
			var hdr=hdrConcat(ftbl,stbl), meta=arrConcat(l.tables[ftbl].meta, l.tables[stbl].meta);
			var obj={}, error=false;
			agr.split("=").trav(function(d,i){
				if(nametoindex(d.trimFB(), hdr)==undefined){
					errorMessage(0,d);
	...
	function senseQuery(str){
		...
		var error=false;
		var tstr=str.getWords().join(" ");
		var tmp=bracket(tstr);
		tmp.rev();
		var brckts={};
		//괄호를 BRACKET으로 치환한다./////////////////////
		...
		//별명이 있는 요소를 별명으로 치환한다.///////////////////////////////
		var stnc=tblstr.getWords();
		var refine=[];
		var subqueries=[];
		...
		function nameValidation(str){
			//subname이 기존의 테이블 명칭과 겹치지 않는지 확인
			for(var el in l.tables){
				if(str==el){
					errorMessage(9,str);
					return false;
		...
		function getTableArea(tstr){
			...
			var before=tstr.substring(0,st), after;
	...
	function hdrConcat(ftbl,stbl){
		var res=[];
		var str=stbl=="JOIN" || stbl=="MULTI" ? "":stbl+".";
		var ftr=ftbl=="JOIN" || ftbl=="MULTI" ? "":ftbl+".";
	...
	function rowcheck(ar,rows){
		//데이터 베이스의 한 행을 받아 조건식과 견주어 보는 함수
		var stack=[];
		//조건에 따른 연산 실행
		rows.trav(function(d,i){
			if(typeof d=="object"){
