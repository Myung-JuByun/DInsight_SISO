...
function pfn_actionLogin() {
	...
	//ajax 호출
	fn_callAjaxFormLogin(actionUrl, formId);
};
//AJAX 파라미터 기준 호출
function fn_callAjaxFormLogin(comboUrl, formId) {
...
//ajax 호출 결과 관련 선처리
function fn_ajax_resultLogin(json, formId) {
...
<div class='login_con' style="background:url('<c:url value='/images/main/bg_login.gif'/>')">
	<form:form method="post" id="loginForm" name="loginForm">
		<div>
			<input style="width:140px; ime-mode:inactive;" class="txt_id required" type="text" id="login_id" name="login_id" title="로그인 아이디" onkeypress="checkKey(event)">
			<input style="width:140px; ime-mode:inactive;" class="txt_pw required" type="password" id="login_Passwd" name="login_Passwd" title="로그인 비밀번호" onkeypress="checkKey(event)">
	    </div>
		<div class="login_btn button"> <!-- <a href="javascript:pfn_actionLogin();"><img src="<c:url value='/images/main/btn_login.gif'/>" alt="로그인" /></a> -->
			<img src="/images/main/btn_login.gif" alt="로그인" onclick="return pfn_actionLogin()"/>
		</div>
		<div id="join_form" style="position:absolute; top:165px; left:273px; width:70px;"><img src="/images/main/btn_join.gif" onclick="joinPop()"/></div>
	 </form:form>
	 <div id="info_msg"></div>
	 <div id="copyrihgt"></div>
</div>
